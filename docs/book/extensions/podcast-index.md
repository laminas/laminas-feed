...
| `addPodcastIndexValue()`             | Expects two arguments: First, an array of the value attributes, containing the required keys `type` and `method` and the optional key `suggested`. As second argument, and array of one or more `valueRecipients` entries is expected. Each entry must be of type array, containing the required keys `type`, `address` and `split` and the optional keys `name`, `customKey`, `customValue` and `fee`. You may consider the example further below. |
